...
public class UInt8ArrayTest extends TestCase {
	...
	public void testMakeCount() {
		...
		AssertArrays.assertEquals(0, array.uInt8At(0));
	...
	public void testUInt8At() {
		...
		AssertArrays.assertEquals(0, a.uInt8At(0));
		AssertArrays.assertEquals(1, a.uInt8At(1));
		AssertArrays.assertEquals(127, a.uInt8At(2));
		AssertArrays.assertEquals(255, a.uInt8At(3));
	...
	public void testCount() {
		AssertArrays.assertEquals(0, AssertArrays.makeUInt8ArrayEmpty().count());
		AssertArrays.assertEquals(1, UInt8Array.make(new short[] { 0 }).count());
		AssertArrays.assertEquals(2, UInt8Array.make(new short[] { 0, 1 }).count());
	}
	...
	public void testStoreInteger() {
		...
		tri.storeInteger(0, IntegerValue.make(UINT8_MIN_VALUE));
		assertTrue(tri.uInt8At(0) == UINT8_MIN_VALUE);
		tri.storeInteger(1, IntegerValue.make(1));
		assertEquals(tri.uInt8At(1), 1);
		tri.storeInteger(2, IntegerValue.make(UINT8_MAX_VALUE));
		assertTrue(tri.uInt8At(2) == UINT8_MAX_VALUE);

		// Store integer values outside of spec
		try {
			tri.storeInteger(0, IntegerValue.make(UINT8_MIN_VALUE - 1));
			fail("MIN_VALUE - 1");
		} catch (IllegalArgumentException e) {
			// expected
		}
		try {
			tri.storeInteger(2, IntegerValue.make(UINT8_MAX_VALUE + 1));
			fail("MAX_VALUE + 1");
		} catch (IllegalArgumentException e) {
			//expected
		}
	...
	public void testStoreValue() {
		...
		// Store integer values within spec
		tri.storeValue(0, IntegerValue.make(UINT8_MIN_VALUE));
		assertTrue(tri.uInt8At(0) == UINT8_MIN_VALUE);
		tri.storeValue(1, IntegerValue.make(1));
		assertEquals(tri.uInt8At(1), 1);
		tri.storeValue(2, IntegerValue.make(UINT8_MAX_VALUE));
		assertTrue(tri.uInt8At(2) == UINT8_MAX_VALUE);

		// Store integer values outside of spec
		try {
			tri.storeValue(0, IntegerValue.make(UINT8_MIN_VALUE - 1));
			fail("MIN_VALUE - 1");
		} catch (IllegalArgumentException e) {
			// expected
		}
		try {
			tri.storeValue(2, IntegerValue.make(UINT8_MAX_VALUE + 1));
			fail("MAX_VALUE + 1");
		} catch (IllegalArgumentException e) {
			// expected
		}
	...
	public void testStoreAll() {
		...
		array = AssertArrays.makeUInt8Array12345();
		try {
			array.storeAll(IntegerValue.make(UINT8_MAX_VALUE + 1), 1);
			fail();
		} catch (IllegalArgumentException e) {
			// expected
		}
	...
	public void testIsEqual() {
		...
		// single value
		assertFalse(UInt8Array.make(chars1).isEqual(IntegerValue.make(1)));
	...
	public void testIndexOf() {
		int index = AssertArrays.makeUInt8ArrayEmpty().indexOf(IntegerValue.make(1), 0, 1);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array1().indexOf(IntegerValue.make(1), 0, 1);
		assertEquals(0, index);

		index = AssertArrays.makeUInt8Array1().indexOf(IntegerValue.make(1), 0, 0);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12345().indexOf(IntegerValue.make(1), 0, 1);
		assertEquals(0, index);

		index = AssertArrays.makeUInt8Array12345().indexOf(IntegerValue.make(1), 0, 2);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12345().indexOf(IntegerValue.make(1), 1, 1);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12345().indexOf(IntegerValue.make(5), 0, 1);
		assertEquals(4, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), 0, 1);
		assertEquals(1, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), 0, 2);
		assertEquals(3, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(1), -1, -1);
		assertEquals(4, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), -1, -1);
		assertEquals(3, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), -1, 1);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), -1, -2);
		assertEquals(1, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), -1, -3);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), -3, -1);
		assertEquals(1, index);
		...
		try {
			AssertArrays.makeUInt8Array12321().indexOf(IntegerValue.make(2), -6, 1);
			fail();
		} catch (IndexOutOfBoundsException e) {
			// expected
		}
		...
		try {
			AssertArrays.makeUInt8Array12321().indexOf(IEEE64Value.make(2.0f), -3, 1);
			fail();
		} catch (ClassCastException e) {
			// expected
		}
	...
	public void testIndexPast() {
		int index = AssertArrays.makeUInt8ArrayEmpty().indexPast(IntegerValue.make(1), 0, 1);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array1().indexPast(IntegerValue.make(1), 0, 1);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array1().indexPast(IntegerValue.make(1), 0, 0);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12345().indexPast(IntegerValue.make(1), 0, 1);
		assertEquals(1, index);

		index = AssertArrays.makeUInt8Array12345().indexPast(IntegerValue.make(1), 0, 2);
		assertEquals(2, index);

		index = AssertArrays.makeUInt8Array12345().indexPast(IntegerValue.make(1), 1, 1);
		assertEquals(1, index);

		index = AssertArrays.makeUInt8Array12345().indexPast(IntegerValue.make(5), 0, 1);
		assertEquals(0, index);

		index = AssertArrays.makeUInt8Array12345().indexPast(IntegerValue.make(5), 3, 1);
		assertEquals(3, index);

		index = AssertArrays.makeUInt8Array12345().indexPast(IntegerValue.make(5), 4, 1);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), 0, 1);
		assertEquals(0, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), 0, 2);
		assertEquals(2, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(1), -1, -1);
		assertEquals(3, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -1, -1);
		assertEquals(4, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -1, 1);
		assertEquals(4, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -1, 2);
		assertEquals(-1, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -1, -2);
		assertEquals(2, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -1, -3);
		assertEquals(0, index);

		index = AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -3, -1);
		assertEquals(2, index);
		...
		try {
			AssertArrays.makeUInt8Array12321().indexPast(IntegerValue.make(2), -6, 1);
			fail();
		} catch (IndexOutOfBoundsException e) {
			// expected
		}
		...
		try {
			AssertArrays.makeUInt8Array12321().indexPast(IEEE64Value.make(2.0), -3, 1);
			fail();
		} catch (ClassCastException e) {
			// expected
		}
