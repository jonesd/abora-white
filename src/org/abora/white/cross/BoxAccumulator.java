...
import org.abora.white.spaces.cross.BoxStepper;
import org.abora.white.spaces.cross.CrossSpace;
import org.abora.white.spaces.cross.GenericCrossRegion;
...
public class BoxAccumulator extends Accumulator {
	...
	protected static InstanceCache SomeAccumulators = InstanceCache.make(8);
	/*
	udanax-top.st:11626:
	...
	*/
	...
	public BoxAccumulator(CrossSpace space, int expectedBoxCount) {
		super();
		mySpace = space;
		myRegions = PtrArray.make(space.axisCount() * expectedBoxCount);
		myIndex = 0;
		/*
	...
	public boolean splitUnion(int added, int current, int stop) {
		...
		while (dimension + 1 < mySpace.axisCount()) {
			...
			//			TODO thingToDo(); /* Add protocol for tri-delta: gives triple (a-b, a&b, b-a) */
	...
	public void mergeBoxes() {
		//		TODO Ravi.thingToDo(); /* hash lookup */
		for (int i = 0; i < myIndex; i++) {
			if (!(isDeleted(i))) {
