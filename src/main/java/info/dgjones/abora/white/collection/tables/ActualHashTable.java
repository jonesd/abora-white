...
import info.dgjones.abora.white.collection.settable.TableEntry;
...
import info.dgjones.abora.white.tabtool.LPPrimeSizeProvider;
...
public class ActualHashTable extends HashTable {
	...
	*/
	...
	public Heaper atStore(Position key, Heaper aHeaper) {
		int offset;
		TableEntry entry;
		TableEntry prev;
		if (aHeaper == null) {
	...
	public Heaper atIntStore(IntegerValue key, Heaper aHeaper) {
		int offset;
		TableEntry entry;
		TableEntry prev;
		if (aHeaper == null) {
	...
	public ScruTable subTable(XnRegion region) {
		HashTable newTable = HashTable.make(myCoordinateSpace, region.count());
		TableStepper elements = stepper();
		try {
			Heaper elemValue;
			while ((elemValue = (Heaper) elements.fetch()) != null) {
				if (region.hasMember(elements.position())) {
					newTable.atStore(elements.position(), elemValue);
				}
				elements.step();
			}
		} finally {
	...
	public int fastHash() {
		int result = getClass().hashCode() + myTally;
		for (int i = 0; i < myHashEntries.count(); i++) {
			TableEntry entry = (TableEntry) (myHashEntries.fetch(i));
			while (entry != null) {
	...
	public ScruTable copy() {
		return new ActualHashTable(myHashEntries, count().asInt32(), coordinateSpace());
		/*
	...
	public void receiveHashTable(Rcvr rcvr) {
		...
		if (myCoordinateSpace.isEqual(IntegerSpace.make())) {
			for (int i = 0; i < myTally; i++) {
				...
				storeEntry((TableEntry.make(index, rcvr.receiveHeaper())));
			}
		} else {
			for (int i = 0; i < myTally; i++) {
				Position key;
	...
	public void sendHashTable(Xmtr xmtr) {
		if (myCoordinateSpace.isEqual(IntegerSpace.make())) {
			...
			try {
				Heaper result;
				while ((result = stepper.fetch()) != null) {
					IntegerValue index = stepper.index();
					xmtr.sendIntegerVar(index);
					xmtr.sendHeaper(result);
					stepper.step();
				}
			} finally {
				destroy();
	...
	//	public void inspect() {
	//		return InspectorView.open((HashTableInspector.inspect(this)));
	//		/*
	...
	public static HashTable make(CoordinateSpace cs, IntegerValue size) {
		return new ActualHashTable(((SharedPtrArray) SharedPtrArray.make((LPPrimeSizeProvider.make().uInt32PrimeAfter((size.asInt32()))))), 0, cs);
		/*
