 ...
 * $Id$
 */
package info.dgjones.abora.white.edge;
...
import info.dgjones.abora.white.edgeregion.EdgeAccumulator;
import info.dgjones.abora.white.edgeregion.EdgeSimpleRegionStepper;
import info.dgjones.abora.white.edgeregion.EdgeStepper;
 ...
 */
public abstract class EdgeManager extends Heaper {
	/*
	udanax-top.st:18173:
	...
	*/
	...
	public EdgeStepper singleEdgeStepper(Position pos) {
		return EdgeStepper.make(true, (posTransitions(pos)));
		/*
	...
	public IntegerValue count(XnRegion region) {
		...
		return IntegerValue.make(transitionsCount(region) / 2);
		/*
		...
		if ((isBoundedLeft(me)) && (isBoundedLeft(other))) {
			if ((isBoundedRight(me)) && (isBoundedRight(other))) {
				return makeNew(
					...
					((PtrArray) (PrimSpec
						.pointer()
						.arrayWithTwo(((lowerEdge(me)).floor((lowerEdge(other)))), ((upperEdge(me)).ceiling((upperEdge(other))))))));
			} else {
	...
	public void printRegionOn(XnRegion region, PrintWriter oo) {
		if (isEmpty(region)) {
			...
			EdgeStepper edges = edgeStepper(region);
			if (!(isSimple(region))) {
				oo.print("{");
			}
			if (!edges.isEntering()) {
	...
	*/
	 ...
	 */
	public abstract XnRegion makeNew(boolean startsInside, PtrArray transitions);
	/*
	...
	*/
	 ...
	 * Make a new region of the right type
	 */
	public abstract XnRegion makeNew(boolean startsInside, PtrArray transitions, int count);
	/*
